...
class TDRecommendRequest: NSObject {
    ...
    /// 小编推荐 --- 轮播图 -- 精品听单 三部分数据
    func getRequestRecommends(_ finished:@escaping (_ result: AnyObject?, _ error: NSError?) -> ()) {
    ...
    /// 分类 -猜你喜欢 -听北京--热门推荐
    func getRequestHotAndGuess(_ finished:@escaping (_ result: AnyObject?, _ error: NSError?) -> ()) {
    ...
    /// 现场直播
    func getRequestLiveRecommend(_ finished:@escaping (_ result: AnyObject?, _ error: NSError?) -> ()){
